...
def format_utc_time(iso_str: str) -> str:
    """格式化原始 UTC 时间为干净的 YYYY-MM-DD HH:MM:SS 字符串"""
    if not iso_str:
        return ""
...
def load_stores(stores_path: Path = STORES_FILE) -> list[dict[str, str]]:
    ...
    for idx, item in enumerate(data, start=1):
        ...
        stores.append({"name": name, "domain": domain, "client_id": client_id, "client_secret": client_secret})
    return stores
...
def fetch_access_token(domain: str, client_id: str, client_secret: str) -> str:
    ...
    for attempt in range(1, 4):
        try:
            ...
            with _token_lock:
                _token_cache[domain] = {"token": access_token, "expires_at": time.time() + expires_in}
            return access_token
...
def run_check() -> None:
    ...
    log(f"===== 本轮巡检结束，共处理 {len(all_flagged_orders)} 条异常订单 =====")
